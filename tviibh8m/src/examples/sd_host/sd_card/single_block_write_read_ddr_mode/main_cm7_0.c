...
int main(void)
{
    ...
    // Send command CMD6 to set DDR in microSD
    Cy_SD_Host_SetBusSpeedMode((stc_SDHC_t *)CY_SDHC_TYPE,CY_SD_HOST_BUS_SPEED_DDR50,&SDIF0_context);
    // Change clock
    Cy_SD_Host_SdCardChangeClock((stc_SDHC_t *)CY_SDHC_TYPE, CY_SD_HOST_CLK_50M);                            

    Cy_SysLib_Delay(1);
    Cy_SysTick_DelayInUs(50000);
    
    // Change controller settings for DDR
    refgen = CY_SDHC_TYPE->CORE.unHOST_CTRL2_R.u16Register;     // Get current contents
    refgen &= ~0x0000007;                                       // Clear bits 2:0
    refgen |= 0x0000004;                                        // Set bit 2, DDR50
    CY_SDHC_TYPE->CORE.unHOST_CTRL2_R.u16Register = refgen;     // Set register
    
    Cy_SysLib_Delay(1);
    Cy_SysTick_DelayInUs(50000);
    
    refgen = CY_SDHC_TYPE->CORE.unGP_OUT_R.u32Register;         // Get current contents
    refgen |= 0x00003C0;                                        // Set bits 9-6, Card clock IN and OUT delays for DDR50
    CY_SDHC_TYPE->CORE.unGP_OUT_R.u32Register = refgen;         // Set register
