...
int main(void)
{
    ...
    g_UartHalfFifoSize =  (Cy_SCB_GetFifoSize(CY_USB_SCB_TYPE) / 2ul);
    ...
    for(;;)
    {
        ...
        while(Cy_SCB_UART_GetNumInRingBuffer(CY_USB_SCB_TYPE, &g_stc_uart_context) < g_UartHalfFifoSize);

        /* Copy from the ring buffer to the local buffer */
        Cy_SCB_UART_Receive(CY_USB_SCB_TYPE, &g_uart_in_data[0], g_UartHalfFifoSize, &g_stc_uart_context);
    }
}
...
void UART_Init_With_Ringbuffer(uint32_t boadrate, uint32_t sourceFreq)
{
    ...
    Cy_SCB_UART_RegisterCallback(CY_USB_SCB_TYPE, Scb_UART_Event, &g_stc_uart_context);
    Cy_SCB_UART_StartRingBuffer(CY_USB_SCB_TYPE, (void *)g_uart_rx_ring, UART_RING_BUF_SIZE, &g_stc_uart_context);
...
void Scb_UART_Event(uint32_t locEvents)
{
    ...
    case CY_SCB_UART_RECEIVE_DONE_EVENT:
        ...
        Cy_SCB_UART_Transmit(CY_USB_SCB_TYPE, &g_uart_in_data[0], g_UartHalfFifoSize, &g_stc_uart_context);
        break;
