...
{
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x001F,   PHY_ADDR, CORE_FREQ_IN_MHZ);
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, u16RegNo, PHY_ADDR, CORE_FREQ_IN_MHZ);
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x401F,   PHY_ADDR, CORE_FREQ_IN_MHZ);
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, u16Data,  PHY_ADDR, CORE_FREQ_IN_MHZ);
}
...
static uint16_t PHY_DP83867IR_ReadExtendedReg(cy_pstc_eth_type_t pstcEth, uint16_t u16RegNo, uint8_t u8PHYAddr)
{
    ...
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, u16RegNo, PHY_ADDR, CORE_FREQ_IN_MHZ);
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x401F,   PHY_ADDR, CORE_FREQ_IN_MHZ);
    return (Cy_EthIf_PhyRegRead(ETH_REG_BASE, 0x0E, PHY_ADDR, CORE_FREQ_IN_MHZ));
}
...
static void InitPHY_DP83867IR (void)
{
    ...
    Cy_EthIf_WaitUsec(30000, CORE_FREQ_IN_MHZ);                                  /** Required delay of 300 ms to get PHY back to Run state after Reset	*/
    ...
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x10, 0x5028, PHY_ADDR, CORE_FREQ_IN_MHZ); /** Disable auto neg for MDI/MDI-X **/
#elif EMAC_LINKSPEEP == ETH_LINKSPEED_1000
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x00, 0x0140, PHY_ADDR, CORE_FREQ_IN_MHZ); /** 1000M, Full Duplex and Auto Negotiation OFF  */
#endif
...
#endif
//    Cy_EthIf_WaitUsec(30000, CORE_FREQ_IN_MHZ);

    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x1F, 0x4000, PHY_ADDR, CORE_FREQ_IN_MHZ); /** Reg CTRl: perform restart */ 
    Cy_EthIf_WaitUsec(30000, CORE_FREQ_IN_MHZ);                                  /** Required delay of 300 ms to get PHY back to Run state after Reset */
}
