...
static void AdcIntHandler(void)
{
    ...
    /* Get the result(s) */
    Cy_Adc_Channel_GetResult(&CY_ADC_MACRO->CH[ADC_LOGICAL_CHANNEL], &resultBuff[0], &statusBuff[0]);
...
static uint16_t AdcReadChannelData(cy_en_adc_pin_address_t channelAddress)
{
    ...
    //for(uint8_t i = 0u; i < ADC_CH_NUM_OF_ITERATION; i++)
    {
...
static cy_en_adc_status_t AdcCalculateSoftTrim(volatile stc_PASS_SAR_t * base)
{
    ...
    AdcPrintLog(" Starting Offset value\t: %d \n\r", analogCalValue.offset);
    ...
    for(int8_t offset=ADC_CAL_OFFSET_MAX_VALUE; offset>=ADC_CAL_OFFSET_MIN_VALUE; offset--)
    {
    ...
    for(int8_t offset=ADC_CAL_OFFSET_MIN_VALUE; offset<=ADC_CAL_OFFSET_MAX_VALUE; offset++)
    {
        /* Update the offset value in ALT_CAL */
        tempCalValue.offset = offset;
        Cy_Adc_SetAltAnalogCalibrationValue(base, &tempCalValue);
    ...
    if(overallCalibration == true)
    {
        ...
        AdcPrintLog(" final offset value A : %d \n\r", finalOffset);
    }
    ...
    if(overallCalibration == true)
    {
        ...
        AdcPrintLog(" final gain value B : %d \n\r", finalGain);
    }
