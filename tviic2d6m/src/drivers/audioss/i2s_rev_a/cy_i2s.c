...
cy_en_i2s_status_t Cy_I2S_Init(volatile stc_I2S_I2S_STRUCT_t * base, cy_stc_i2s_config_t const * config)
{
    ...
    {
        Cy_I2S_Tx_Init(&(base->I2S_TX_STRUCT), config);
        Cy_I2S_Rx_Init(&(base->I2S_RX_STRUCT), config);
    }
...
cy_en_i2s_status_t Cy_I2S_Tx_Init(volatile stc_I2S_I2S_STRUCT_I2S_TX_STRUCT_t * base, cy_stc_i2s_config_t const * config)
{
    ...
    un_I2S_I2S_STRUCT_I2S_TX_STRUCT_TX_IF_CTL_t temp_IF_CTL = {0};
    un_I2S_I2S_STRUCT_I2S_TX_STRUCT_TX_CTL_t    temp_TX_CTL = {0};
    ...
    if((NULL != base) && (NULL != config))
    {
        ...
        temp_IF_CTL.stcField.u1WS_POLARITY  = config->txWsPolarity;
        temp_IF_CTL.stcField.u1CH0_EN       = config->txChannel0Enable;
        temp_IF_CTL.stcField.u1CH1_EN       = config->txChannel1Enable;
        ...
        if (config->txEnabled)
        {
            ...
            base->unTX_FIFO_CTL.stcField.u6TRIGGER_LEVEL = config->txFifoTriggerLevel;
        }
...
cy_en_i2s_status_t Cy_I2S_Rx_Init(volatile stc_I2S_I2S_STRUCT_I2S_RX_STRUCT_t * base, cy_stc_i2s_config_t const * config)
{
    ...
    un_I2S_I2S_STRUCT_I2S_RX_STRUCT_RX_IF_CTL_t temp_IF_CTL = {0};
    un_I2S_I2S_STRUCT_I2S_RX_STRUCT_RX_CTL_t    temp_RX_CTL = {0};
    ...
    if((NULL != base) && (NULL != config))
    {
        ...
        temp_IF_CTL.stcField.u1WS_POLARITY  = config->rxWsPolarity;
        temp_IF_CTL.stcField.u1LATE_SAMPLE  = config->rxLateSampleDelay;
        temp_IF_CTL.stcField.u1CH0_EN       = config->rxChannel0Enable;
        temp_IF_CTL.stcField.u1CH1_EN       = config->rxChannel1Enable;
        ...
        if (config->rxEnabled)
        {
            ...
            temp_RX_CTL.stcField.u1WORD_SIGN_EXTEND = config->rxSignExtension;
        ...
        if (config->rxEnabled)
        {
            ...
            base->unRX_FIFO_CTL.stcField.u6TRIGGER_LEVEL = config->rxFifoTriggerLevel;
        }
...
{
    Cy_I2S_Tx_DeInit(&(base->I2S_TX_STRUCT));
    Cy_I2S_Rx_DeInit(&(base->I2S_RX_STRUCT));
}
...
cy_en_syspm_status_t Cy_I2S_DeepSleepCallback(cy_stc_syspm_callback_params_t * callbackParams)
{
    ...
    stc_I2S_I2S_STRUCT_I2S_TX_STRUCT_t * locTxBase = (stc_I2S_I2S_STRUCT_I2S_TX_STRUCT_t*) callbackParams->base;
    stc_I2S_I2S_STRUCT_I2S_RX_STRUCT_t * locRxBase = (stc_I2S_I2S_STRUCT_I2S_RX_STRUCT_t*) callbackParams->base;
    ...
    switch(callbackParams->mode)
    {
        ...
        case CY_SYSPM_BEFORE_ENTER:
            ...
            *locRxState = Cy_I2S_GetRxCurrentState(locRxBase);          /* Store I2S state */
            ...
            if (0UL != (*locTxState & I2S_I2S_STRUCT_I2S_TX_STRUCT_TX_FIFO_CTL_ACTIVE_Msk))
            {
                Cy_I2S_DisableTx(locTxBase);                            /* Stop TX operation */
            }
            if (0UL != (*locRxState & I2S_I2S_STRUCT_I2S_RX_STRUCT_RX_FIFO_CTL_ACTIVE_Msk))
            {
