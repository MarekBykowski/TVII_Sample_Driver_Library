...
int main(void)
{
    ...
    Cy_SCB_UART_StartRingBuffer(CY_USB_SCB_TYPE, (void *)g_uart_rx_ring, E_UART_RING_BUF_SIZE, &g_stc_uart_context);
#endif
    ...
    for(;;)
    {
        ...
        uint32_t num = Cy_SCB_UART_GetNumInRingBuffer(CY_USB_SCB_TYPE, &g_stc_uart_context);
        if (num != 0) {
            Cy_SCB_UART_Receive(CY_USB_SCB_TYPE, &g_uart_user_buf[0], num, &g_stc_uart_context);
            Cy_SCB_UART_Transmit(CY_USB_SCB_TYPE, &g_uart_user_buf[0], num, &g_stc_uart_context);
        }
#endif
    }
}
...
void Scb_UART_Event(uint32_t locEvents)
{
    switch (locEvents) {
        ...
        case CY_SCB_UART_RECEIVE_DONE_EVENT:
            ...
            /* (3) Interrupt & Receive by threshold byte unit (E_UART_RECV_THRESHOLD) */
            Cy_SCB_UART_Transmit(CY_USB_SCB_TYPE, &g_uart_in_data[0], E_UART_RECV_THRESHOLD, &g_stc_uart_context);
            Cy_SCB_UART_Receive(CY_USB_SCB_TYPE, &g_uart_in_data[0], E_UART_RECV_THRESHOLD, &g_stc_uart_context);
#endif
            /* Re-Enable Interrupt */
...
#if 0
    uint32_t    cmd;
    cmd = Cy_SCB_UART_Get(CY_USB_SCB_TYPE);
    while (0UL == Cy_SCB_UART_Put(CY_USB_SCB_TYPE, cmd)) {}
#endif
#if 1
    Cy_SCB_UART_GetArray(CY_USB_SCB_TYPE, g_uart_in_data, rx_num);
    Cy_SCB_UART_PutArray(CY_USB_SCB_TYPE, g_uart_in_data, rx_num);
#endif
#if 0
    Cy_SCB_UART_GetArrayBlocking(CY_USB_SCB_TYPE, g_uart_in_data, rx_num);
    Cy_SCB_UART_PutArrayBlocking(CY_USB_SCB_TYPE, g_uart_in_data, rx_num);
    Cy_SCB_UART_PutString(CY_USB_SCB_TYPE, (char_t *)g_uart_in_data);
#endif
}
