...
cy_semihosting_en_result_t Cy_Semihosting_InitAll(volatile stc_SCB_t *pScbInstance, uint32_t u32Bitrate, cy_semihosting_wdg_handle_callout_t pfnWatchdogCallout, bool bIsDummyInit)
{
...
#ifdef CY_USB_SCB_TYPE

    cy_semihosting_en_result_t enSemihostingResult = CY_SEMIHOSTING_SUCCESS;
    
    // This function only supports the SCB instance which is defined in the BSP header
    if (pScbInstance != CY_USB_SCB_TYPE)
    {
        return CY_SEMIHOSTING_BAD_PARAM;
    }
    ...
    if(bIsDummyInit == false)
    {
        ...
        Cy_GPIO_Pin_FastInit(CY_USB_SCB_UART_TX_PORT, CY_USB_SCB_UART_TX_PIN, CY_GPIO_DM_STRONG, 0, CY_USB_SCB_UART_TX_MUX);
        Cy_GPIO_Pin_FastInit(CY_USB_SCB_UART_RX_PORT, CY_USB_SCB_UART_RX_PIN, CY_GPIO_DM_HIGHZ,  0, CY_USB_SCB_UART_RX_MUX);
    ...
    // CY_USB_SCB... defines from BSP header are needed, but they do not seem to be defined.
