...
{
    if((NULL == base) || (NULL == config) || (NULL == context) || (NULL == config->pDllCfg))
    {
    ...
    tempCtl.stcField.u1CLOCK_IF_SEL       = config->rxClk;
    ...
    /* Set up DLL configuration */
    cy_en_smif_status_t status;
    status =Cy_SMIF_DllConfig(base, config->pDllCfg);
...
cy_en_smif_status_t Cy_SMIF_DllConfig(volatile cy_stc_smif_reg_t *base, const cy_stc_smif_dll_config_t* pDllCfg)
{
    const uint32_t gPllClockFreqBoarderForDLL[] = {160000000ul, 180000000ul, 266000000ul, 333000000ul, 400000000ul};

    if((NULL == base) || (NULL == pDllCfg))
    {
        return CY_SMIF_BAD_PARAM;
    }

    un_SMIF_CORE_CTL2_t tempCtl2;
    tempCtl2 = base->unCTL2;

    uint32_t pllRangeIdx;
    uint32_t pllRangeCount = sizeof(gPllClockFreqBoarderForDLL)/sizeof(gPllClockFreqBoarderForDLL[0]) - 1ul;
    for(pllRangeIdx = 0; pllRangeIdx < pllRangeCount; pllRangeIdx+=1ul)
    {
        if((gPllClockFreqBoarderForDLL[pllRangeIdx] <= pDllCfg->pllFreq) && (pDllCfg->pllFreq <= gPllClockFreqBoarderForDLL[pllRangeIdx+1]))
        {
            tempCtl2.stcField.u2DLL_SPEED_MODE = pllRangeIdx;
            break;
        }
    }
    if(pllRangeIdx == pllRangeCount)
    {
        return CY_SMIF_BAD_PARAM;
    }

    tempCtl2.stcField.u2CLKOUT_DIV         = pDllCfg->mdlOutDiv;
    tempCtl2.stcField.u4MDL_TAP_SEL        = pDllCfg->mdlTapSel;
    tempCtl2.stcField.u2RX_CAPTURE_MODE    = pDllCfg->rxCapMode;
    tempCtl2.stcField.u2RX_CHASE_MARGIN    = 2ul;                 /* The default value expected to work for all situations */
    tempCtl2.stcField.u1TX_SDR_EXTRA_SETUP = pDllCfg->txSdrExtra;

    base->unCTL2 = tempCtl2;

    /* Read the register to flush the buffer */
    (void)base->unCTL2.u32Register;

    return CY_SMIF_SUCCESS;
}

void Cy_SMIF_SetDllOutDivider(volatile cy_stc_smif_reg_t *base, cy_en_cy_smif_mdl_clk_out_div_t div)
{
    base->unCTL2.stcField.u2CLKOUT_DIV = div;
}

bool Cy_SMIF_IsDllLocked(volatile cy_stc_smif_reg_t *base)
{
    return (bool)(base->unSTATUS.stcField.u1DLL_LOCKED == 1ul);
}


/*******************************************************************************
* Function Name: Cy_SMIF_SetCaptureConfig
****************************************************************************//**
*
* This function initializes the SMIF capture settings. The user
* must ensure that the SMIF ip should be disabled while this function
* is called. 
*
* As parameters, this function takes the SMIF register base address and a
* slave device number with the configuration needed for the SMIF block,
* stored in a config.
*
* \param base
* Holds the base address of the SMIF block registers.
*
* \param slave
* Passes a selected device on the SMIF instance.
*
* \param isHyperBus
* True for hyperbus mode and false for spi devices.
*
* \note Temperory function to adapt changes for new ip.
*
* \return
*     - \ref CY_SMIF_BAD_PARAM
*     - \ref CY_SMIF_SUCCESS
*
*******************************************************************************/
cy_en_smif_status_t Cy_SMIF_SetCaptureConfig(volatile cy_stc_smif_reg_t *base, 
                                            cy_en_smif_slave_select_t slave, 
                                            bool isHyperBus)
{
    if(isHyperBus == true)
    {
        // Clock divide by 8, HyperBus or Octal mode, RX Chase Margin
        base->unCTL2.u32Register = (2 << 4) | (2 << 20) | (2 << 22);
        
        /* Configure capture scheme, right now support hyperbus and qspi(without dlp) */
        if(slave == CY_SMIF_SLAVE_SELECT_0)
        {
            // DDR_PIPELINE_POS_DAT, DDR_SWAP_BYTES
            base->DEVICE[0].unRX_CAPTURE_CONFIG.u32Register = (1 << 24) | (1 << 25);
            
            // Write enable, device enable
            base->DEVICE[0].unCTL.u32Register = (1 << 31) | (1 << 0);
        }
        else
        {
            // DDR_PIPELINE_POS_DAT, DDR_SWAP_BYTES
            base->DEVICE[1].unRX_CAPTURE_CONFIG.u32Register = (1 << 24) | (1 << 25);
            
            // Write enable, device enable
            base->DEVICE[1].unCTL.u32Register = (1 << 31) | (1 << 0);
        }
    }
    else
    {
        // Clock divide by 8, RX Chase Margin
        base->unCTL2.u32Register = (2 << 4) | (2 << 22);
        base->DEVICE[0].unRX_CAPTURE_CONFIG.u32Register = 0;
    }
...
void Cy_SMIF_DeInit(volatile cy_stc_smif_reg_t *base)
{
    ...
    */
    ...
    base->unCTL2.u32Register = CY_SMIF_CTL2_REG_DEFAULT;
...
cy_en_smif_status_t Cy_SMIF_SetDelayLineForFreq(volatile cy_stc_smif_reg_t *base, uint8_t freqMhz, bool isDDR, uint8_t *calculatedTap)
{
    ...
    //rmkn ww2032: not there static const float32_t minDelayPerTapInNs[CY_SMIF_DRV_SMIF0_DELAY_LINES_NR] = {0.0559, 0.1197, 0.1935, 0.3973};
    cy_en_smif_status_t status = CY_SMIF_SUCCESS;
    uint8_t tapNumCalculated = 0;
    uint8_t delayLine = 0;
    uint8_t maxTaps = CY_SMIF_GetDelayTapsNumber(base);

#if 0 // rmkn ww2032: not there
...
{
    base->unDLP_CTL.stcField.u16DLP = dlp;
}
...
{
    return (uint16_t)(base->unDLP_CTL.stcField.u16DLP);
}
...
cy_en_smif_status_t Cy_SMIF_SetMasterDLP_Size(volatile cy_stc_smif_reg_t *base, uint32_t size)
{
    ...
    base->unDLP_CTL.stcField.u4DLP_SIZE = size - 1ul;
...
{
    return (uint8_t)(base->unDLP_CTL.stcField.u4DLP_SIZE);
}
...
cy_en_smif_status_t Cy_SMIF_SetMasterDLP_WarnLevel(volatile cy_stc_smif_reg_t *base, uint32_t warnLevel)
{
    ...
    base->unDLP_CTL.stcField.u4DLP_WARNING_LEVEL = warnLevel;
...
{
    return (uint8_t)(base->unDLP_CTL.stcField.u4DLP_WARNING_LEVEL);
}
...
uint8_t Cy_SMIF_GetTapNumCapturedCorrectDLP(volatile cy_stc_smif_reg_t *base, uint8_t bit)
{
    ...
    if(bit < 4u)
    {
        return(base->unDLP_STATUS0.au8Byte[bit]);
    }
    else
    {
        return(base->unDLP_STATUS1.au8Byte[bit - 4u]);
    }
}
...
{
    // This is a dummy function to keep backward compatibility
    return CY_SMIF_SUCCESS;
}
...
cy_en_smif_status_t Cy_SMIF_Set_DelayTapSel(volatile void *smif_or_device_base, uint8_t tapSel)
{
    ...
    cy_un_smif_reg_device_capture_config_t tempDelaySEL = ((volatile cy_stc_smif_reg_device_t*)smif_or_device_base)->unRX_CAPTURE_CONFIG;
    tempDelaySEL.stcField.u4POS_SDL_TAP_SEL = tapSel;
    tempDelaySEL.stcField.u4NEG_SDL_TAP_SEL = tapSel;
    ((volatile cy_stc_smif_reg_device_t*)smif_or_device_base)->unRX_CAPTURE_CONFIG = tempDelaySEL;
...
uint8_t Cy_SMIF_Get_DelayTapSel(volatile void *smif_or_device_base)
{
    if((smif_or_device_base == CY_SMIF_DRV_SMIF0_CORE0_DEVICE0) || (smif_or_device_base == CY_SMIF_DRV_SMIF0_CORE0_DEVICE1))
    {
        return (uint8_t)(((volatile cy_stc_smif_reg_device_t*)smif_or_device_base)->unRX_CAPTURE_CONFIG.au8Byte[0]);
    }
    else
    {
        // Bad parameter input.
...
{
    if(capStyle == CY_SMIF_RX_CAP_STYLE_DDR_NORM_SPI)
    {
        device->unRX_CAPTURE_CONFIG.stcField.u1DDR_PIPELINE_POS_DAT = 0ul;
        device->unRX_CAPTURE_CONFIG.stcField.u1DDR_SWAP_BYTES       = 0ul;
    }
    else if((capStyle == CY_SMIF_RX_CAP_STYLE_DDR_HYPERBUS)  || (capStyle == CY_SMIF_RX_CAP_STYLE_DDR_OCTAL_SWAP))
    {
        device->unRX_CAPTURE_CONFIG.stcField.u1DDR_PIPELINE_POS_DAT = 1ul;
        device->unRX_CAPTURE_CONFIG.stcField.u1DDR_SWAP_BYTES       = 1ul;
    }
    else if((capStyle == CY_SMIF_RX_CAP_STYLE_DDR_xSPI) || (capStyle == CY_SMIF_RX_CAP_STYLE_DDR_OCTAL))
    {
        device->unRX_CAPTURE_CONFIG.stcField.u1DDR_PIPELINE_POS_DAT = 1ul;
        device->unRX_CAPTURE_CONFIG.stcField.u1DDR_SWAP_BYTES       = 0ul;
    }
...
void Cy_SMIF_DeviceSetRxCaptureSdr(volatile cy_stc_smif_reg_device_t *device, cy_en_sdr_cap_style_t capStyle)
{
    if(capStyle == CY_SMIF_RX_CAP_STYLE_SDR_POS)
    {
        device->unRX_CAPTURE_CONFIG.stcField.u1SDR_PIPELINE_NEG_DAT = 0ul;
        device->unRX_CAPTURE_CONFIG.stcField.u1SDR_SWAP_BYTES       = 0ul;
    }
    else if(capStyle == CY_SMIF_RX_CAP_STYLE_SDR_NEG_NORM)
    {
        device->unRX_CAPTURE_CONFIG.stcField.u1SDR_PIPELINE_NEG_DAT = 0ul;
        device->unRX_CAPTURE_CONFIG.stcField.u1SDR_SWAP_BYTES       = 1ul;
    }
    else if(capStyle == CY_SMIF_RX_CAP_STYLE_SDR_NEG_PIPED)
    {
        device->unRX_CAPTURE_CONFIG.stcField.u1SDR_PIPELINE_NEG_DAT = 1ul;
        device->unRX_CAPTURE_CONFIG.stcField.u1SDR_SWAP_BYTES       = 1ul;
    }
    else
    {
...
cy_en_smif_status_t Cy_SMIF_SwitchBridge(volatile cy_stc_smif_base_reg_t *baseaddr, bool switchOn)
{
    uint32_t prevStatus = Cy_SysLib_EnterCriticalSection();

    /* Confirming all smif core is not busy */
    for(uint32_t smifCoreIdx = 0; smifCoreIdx < CY_SMIF_CORE_COUNT; smifCoreIdx+=1ul)
    {
        if(Cy_SMIF_IsBusy(&baseaddr->CORE[smifCoreIdx]) == true)
        {
            return CY_SMIF_BAD_STATUS;
        }
    }
    ...
    baseaddr->SMIF_BRIDGE.unCTL.stcField.u1ENABLED = (uint32_t)switchOn;

    /* The void Read of the CTL register to handle buffering */
    (void)baseaddr->SMIF_BRIDGE.unCTL.u32Register;
...
static bool Cy_SMIF_IsBridgeOn(volatile cy_stc_smif_base_reg_t *baseaddr)
{
    return (baseaddr->SMIF_BRIDGE.unCTL.stcField.u1ENABLED == 1ul);
}

cy_en_smif_status_t Cy_SMIF_SetBridgePriority(volatile cy_stc_smif_base_reg_t *baseaddr, cy_en_smif_bridge_xip_space_pri_t pri_ahb_smif0, cy_en_smif_bridge_xip_space_pri_t pri_ahb_smif1, cy_en_smif_bridge_xip_space_pri_t pri_axi_smif0, cy_en_smif_bridge_xip_space_pri_t pri_axi_smif1)
{
    if(Cy_SMIF_IsBridgeOn(baseaddr) == true)
    {
        return CY_SMIF_BAD_STATUS;
    }

    cy_un_smif_reg_bridge_ctl_t tmpCtrReg = {0ul};
    tmpCtrReg.stcField.u1ENABLED           = 0ul;
    tmpCtrReg.stcField.u1ARB_PRI_AHB_SMIF0 = pri_ahb_smif0;
    tmpCtrReg.stcField.u1ARB_PRI_AHB_SMIF1 = pri_ahb_smif1;
    tmpCtrReg.stcField.u1ARB_PRI_AXI_SMIF0 = pri_axi_smif0;
    tmpCtrReg.stcField.u1ARB_PRI_AXI_SMIF1 = pri_axi_smif1;
    baseaddr->SMIF_BRIDGE.unCTL.u32Register = tmpCtrReg.u32Register;
    return CY_SMIF_SUCCESS;
}

static bool Cy_SMIF_IsRegionAddrAligned(uint32_t adder, cy_en_smif_bridge_remap_region_size_t regionSize)
{
    if(((adder & 0x1FFFFFFFul) & (~regionSize)) == 0ul)
    {
        return true;
    }
    {
...
static uint32_t Cy_SMIF_GetSmifCoreIdxFromXipAddr(volatile cy_stc_smif_base_reg_t *baseaddr, uint32_t addr)
{
    uint32_t smifCoreIdx;
    for(smifCoreIdx = 0; smifCoreIdx < CY_SMIF_CORE_COUNT; smifCoreIdx+=1ul)
    {
        if((addr & 0xF0000000ul) == CY_SMIF_GetXIP_Address((void*)&baseaddr->CORE[smifCoreIdx]))
        {
            return smifCoreIdx;
        }
    }
...
cy_en_smif_status_t Cy_SMIF_SetSimpleRemapRegion(volatile cy_stc_smif_base_reg_t *baseaddr, const cy_stc_smif_bridge_remap_t* pCfg)
{
    /* Status Checking */
    if(Cy_SMIF_IsBridgeOn(baseaddr) == true)
    {
    ...
    /* Parameter Checking */
    if(pCfg->regionIdx >= CY_SMIF_BRIGE_REMAP_REGION_COUNT)
    {
        return CY_SMIF_BAD_PARAM;
    }

    if(!Cy_SMIF_IsRegionAddrAligned(pCfg->xipAddr, pCfg->regionSize))
    {
        return CY_SMIF_BAD_PARAM;
    }

    if(!Cy_SMIF_IsRegionAddrAligned(pCfg->phyAddr, pCfg->regionSize))
    {
        return CY_SMIF_BAD_PARAM;
    }

    cy_en_smif_bridge_xip_space_t xipSpace;
    uint32_t xipSmifIdx = Cy_SMIF_GetSmifCoreIdxFromXipAddr(baseaddr, pCfg->xipAddr);
    if(xipSmifIdx == 0ul) // Is SMIF core 0 XIP space?
    {
        xipSpace = CY_EN_BRIDGE_SMIF0_XPI_SPACE;
    }
    else if(xipSmifIdx == 1ul) // Is SMIF core 1 XIP space?
    {
        xipSpace = CY_EN_BRIDGE_SMIF1_XPI_SPACE;
    }
    else
    {
        return CY_SMIF_BAD_PARAM;
    ...
    cy_en_smif_bridge_remap_type_t remapDest;
    uint32_t phySmifIdx = Cy_SMIF_GetSmifCoreIdxFromXipAddr(baseaddr, pCfg->phyAddr);
    if(phySmifIdx == 0ul) // Is target physical SMIF core 0?
    {
        remapDest = CY_EN_BRIGE_REMAP_TYPE_TO_SMIF0;
    }
    else if(phySmifIdx == 1ul) // Is target physical SMIF core 1?
    {
        remapDest = CY_EN_BRIGE_REMAP_TYPE_TO_SMIF1;
    }
    else
    {
        return CY_SMIF_BAD_PARAM;
    ...
    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unCTL.stcField.u2USE_SMIF   = remapDest;
    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unCTL.stcField.u1SMIF_SPACE = xipSpace;

    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unADDR.u32Register = pCfg->xipAddr;
    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unMASK.u32Register = pCfg->regionSize;

    if(remapDest == CY_EN_BRIGE_REMAP_TYPE_TO_SMIF0)
    {
        baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unSMIF0_REMAP.u32Register = pCfg->phyAddr;
    }
    else if(remapDest == CY_EN_BRIGE_REMAP_TYPE_TO_SMIF1)
    {
        baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unSMIF1_REMAP.u32Register = pCfg->phyAddr;
    }
    else
    {
...
static cy_en_smif_bridge_remap_region_size_t Cy_SMIF_GetHalfRegionSize(cy_en_smif_bridge_remap_region_size_t regionSize)
{
    return (cy_en_smif_bridge_remap_region_size_t)((regionSize >> 1ul) + 0x10000000ul);
}

cy_en_smif_status_t Cy_SMIF_SetInterleavingRemapRegion(volatile cy_stc_smif_base_reg_t *baseaddr, const cy_stc_smif_bridge_interleave_remap_t* pCfg)
{
    /* Status Checking */
    if(Cy_SMIF_IsBridgeOn(baseaddr) == true)
    {
    ...
    if(!Cy_SMIF_IsRegionAddrAligned(pCfg->phyAddr0, Cy_SMIF_GetHalfRegionSize(pCfg->regionSize)))
    {
        return CY_SMIF_BAD_PARAM;
    }

    if(!Cy_SMIF_IsRegionAddrAligned(pCfg->phyAddr1, Cy_SMIF_GetHalfRegionSize(pCfg->regionSize)))
    {
        return CY_SMIF_BAD_PARAM;
    }
    ...
    uint32_t phySmif0Idx = Cy_SMIF_GetSmifCoreIdxFromXipAddr(baseaddr, pCfg->phyAddr0);
    uint32_t phySmif1Idx = Cy_SMIF_GetSmifCoreIdxFromXipAddr(baseaddr, pCfg->phyAddr1);
    if(phySmif0Idx == phySmif1Idx)
    {
        return CY_SMIF_BAD_PARAM;
    }

    if(phySmif0Idx >= CY_SMIF_CORE_COUNT)
    {
       return CY_SMIF_BAD_PARAM;
    }

    if(phySmif1Idx >= CY_SMIF_CORE_COUNT)
    {
        return CY_SMIF_BAD_PARAM;
    }

    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unCTL.stcField.u2USE_SMIF   = CY_EN_BRIGE_REMAP_TYPE_INTERLEAVE;
    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unCTL.stcField.u1SMIF_SPACE = xipSpace;

    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unADDR.u32Register = pCfg->xipAddr;
    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unMASK.u32Register = pCfg->regionSize;

    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unSMIF0_REMAP.u32Register = pCfg->phyAddr0;
    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[pCfg->regionIdx].unSMIF1_REMAP.u32Register = pCfg->phyAddr1;
    return CY_SMIF_SUCCESS;
}

cy_en_smif_status_t Cy_SMIF_DeactivateRemapRegion(volatile cy_stc_smif_base_reg_t *baseaddr, uint32_t regionIdx)
{
    /* Status Checking */
    if(Cy_SMIF_IsBridgeOn(baseaddr) == true)
    {
    ...
    /* Parameter Checking */
    if(regionIdx >= CY_SMIF_BRIGE_REMAP_REGION_COUNT)
    {
        return CY_SMIF_BAD_PARAM;
    }

    baseaddr->SMIF_BRIDGE.SMIF_REMAP_REGION[regionIdx].unCTL.stcField.u2USE_SMIF   = CY_EN_BRIGE_REMAP_TYPE_INACTIVE;
