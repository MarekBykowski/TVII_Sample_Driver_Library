...
int main(void)
{
    ...
    {
        /* Start receiving */
        Cy_SCB_UART_Receive(CY_USB_SCB_TYPE, &g_uart_in_data[0], UART_RX_UNIT_BYTE, &g_stc_uart_context);

        /* Wait for receive done */
        while((Cy_SCB_UART_GetReceiveStatus(CY_USB_SCB_TYPE,&g_stc_uart_context) & CY_SCB_UART_RECEIVE_ACTIVE) == CY_SCB_UART_RECEIVE_ACTIVE);
    }
}
...
void Scb_UART_Event(uint32_t locEvents)
{
    ...
    case CY_SCB_UART_RECEIVE_DONE_EVENT:
        ...
        Cy_SCB_UART_Transmit(CY_USB_SCB_TYPE, &g_uart_in_data[0], UART_RX_UNIT_BYTE, &g_stc_uart_context);
        break;
