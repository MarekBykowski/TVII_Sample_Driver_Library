...
cy_en_seglcd_status_t Cy_SegLCD_Init(stc_LCD_t * base, cy_stc_seglcd_config_t const * config)
{
    ...
    if (NULL != base)
    {
        ...
        {
            //rmkn not used uint32_t locLsClk = (uint32_t)((CY_SEGLCD_REV_1(base)) ? CY_SEGLCD_LSCLK_LF : config->lsClk);

            Cy_SegLCD_Disable(base);
            
            /* Calculate the sub-frame and dead-time dividers */
            base->unDIVIDER.u32Register = _VAL2FLD(LCD_DIVIDER_SUBFR_DIV, locSubfr) |
                                _VAL2FLD(LCD_DIVIDER_DEAD_DIV,  locDead);

            /* Set the rest of settings */
            base->unCONTROL.u32Register = _VAL2FLD(LCD_CONTROL_LCD_MODE, config->speed) |
                                _VAL2FLD(LCD_CONTROL_TYPE,     config->wave)  |
                                _VAL2FLD(LCD_CONTROL_OP_MODE,  config->drive) |
                                _VAL2FLD(LCD_CONTROL_BIAS,     config->bias)  |
            /*rmkn commenting            _VAL2FLD(LCD_V2_CONTROL_CLOCK_LS_SEL,  locLsClk)      |*/
...
{
    if (_FLD2BOOL(LCD_CONTROL_LCD_MODE, base->unCONTROL.u32Register))
    {
    ...
    else /* CY_SEGLCD_SPEED_HIGH */
    {
        ...
        for (timeout = CY_SEGLCD_TIMEOUT; (_FLD2BOOL(LCD_CONTROL_LS_EN_STAT, base->unCONTROL.u32Register)) && (0UL != timeout); timeout--)
        {
...
{
    if (_FLD2BOOL(LCD_CONTROL_LCD_MODE, base->unCONTROL.u32Register)) /* CY_SEGLCD_SPEED_HIGH */
    {
        base->unCONTROL.u32Register &= ~LCD_CONTROL_HS_EN_Msk;
    }
    else /* CY_SEGLCD_SPEED_LOW */
    {
        if (_FLD2BOOL(LCD_CONTROL_LS_EN, base->unCONTROL.u32Register))
        {
            uint32_t timeout;
...
*
*******************************************************************************/
cy_en_seglcd_status_t Cy_SegLCD_BarGraph(stc_LCD_t * base,
                                         uint32_t   value,
                                         uint32_t   position,
                       cy_stc_seglcd_disp_t const * display)
{
