...
//------------------------------------------------------------------------------
static int i2sData[64] =
{
    0x0000, 0x085F, 0x10B4, 0x18F8, 0x2120, 0x2924, 0x30FB, 0x389C, 0x3FFF, 0x471C, 0x4DEB, 0x5464, 0x5A81, 0x603B, 0x658B, 0x6A6C, 
    0x6ED9, 0x72CB, 0x7640, 0x7934, 0x7BA2, 0x7D89, 0x7EE6, 0x7FB8, 0x7FFF, 0x7FB8, 0x7EE6, 0x7D89, 0x7BA2, 0x7934, 0x7640, 0x72CB, 
    0x6ED9, 0x6A6C, 0x658B, 0x603B, 0x5A81, 0x5464, 0x4DEB, 0x471C, 0x3FFF, 0x389C, 0x30FB, 0x2924, 0x2120, 0x18F8, 0x10B4, 0x085F,
    0x0000, 0xF7A1, 0xEF4B, 0xE708, 0xDEE0, 0xD6DC, 0xCF05, 0xC764, 0xC001, 0xB8E4, 0xB215, 0xAB9C, 0xA57F, 0x9FC5, 0x9A75, 0x9594, 
};  

#define DATA_NUM_PER_PERIOD (sizeof(i2sTransmitData) / sizeof(uint32_t))
...
cy_stc_i2s_config_t const config_I2S =
{
    ...
    .txClkDiv           = 127u,                   // Divided by 127 -- 1.54MHz
...
{       
    ...
    {
        Cy_I2S_WriteTxData(&CY_AUDIOSS_I2S_TYPE->I2S_TX_STRUCT, i2sData[data_i]);
    }
...
//------------------------------------------------------------------------------
void ReceiveI2sData(uint8_t dataSamples)
{   
    for(uint8_t data_i = 0; data_i <dataSamples; data_i++)
    {
        i2sData[data_i] = Cy_I2S_ReadRxData(&CY_AUDIOSS_I2S_TYPE->I2S_RX_STRUCT);
    }
    Cy_I2S_SetRxTriggerInterruptMask(&CY_AUDIOSS_I2S_TYPE->I2S_RX_STRUCT);
}
...
void AudioSSInterruptHandler(void)
{
    ...
    maskedIntrStatus = Cy_I2S_GetRxInterruptStatusMasked(&CY_AUDIOSS_I2S_TYPE->I2S_RX_STRUCT);
    if((maskedIntrStatus & CY_I2S_INTR_RX_FIFO_TRIGGER) != 0)
    {
        Cy_I2S_ClearRxTriggerInterruptMask(&CY_AUDIOSS_I2S_TYPE->I2S_RX_STRUCT);
        isReceiveReadyFlag = true;
    }
}
...
int main(void)
{
   ...
   // External clock is not supported through software check for resistor setting
#endif
    ...
    /* Enable TX and RX interface */
    Cy_I2S_EnableTx(&CY_AUDIOSS_I2S_TYPE->I2S_TX_STRUCT);
    Cy_I2S_EnableRx(&CY_AUDIOSS_I2S_TYPE->I2S_RX_STRUCT);
    ...
    for(;;)
    {
        uint8_t noOfReceivedBytes = 0;
        noOfReceivedBytes = Cy_I2S_GetNumInRxFifo(&CY_AUDIOSS_I2S_TYPE->I2S_RX_STRUCT);
        
        /* Receive data if receiver is ready */
        if(isReceiveReadyFlag == true)
        {   
            isReceiveReadyFlag = false;
            ReceiveI2sData(noOfReceivedBytes);
        }
        ...
        {   
            ...
            TransmitI2sData(noOfReceivedBytes);
        }    
    }
}
