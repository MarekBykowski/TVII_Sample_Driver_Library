...
#include "drivers\ethernet\cy_ethif_defines.h"
...
//        if (bFrameReceived || bFrameTransmit)
        if (bFrameReceived)
        {
            ...
            #if 0
                /** Debug routine   */
                u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x15, PHY_ADDR, CORE_FREQ_IN_MHZ);       /** Read the RGMII mode status  */            
                
                Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x001F, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** REGCR  */
                Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x0135, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** ADDAR, 0x0086 Delay config register  */
                Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x401F, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** REGCR, will force next write/read access non incremental  */
                u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x0E, PHY_ADDR, CORE_FREQ_IN_MHZ);       /** Read the RGMII mode status  */    
                /** Debug Routine ends  */
            #endif
...
static void InitPHY_DP83867IR (void)
{
    ...
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x1F, 0x8000, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** Reg CTRl: Perform a full reset, including All registers   */ 
...
#if EMAC_LINKSPEED == ETH_LINKSPEED_10
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x00, 0x0100, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** 10M, Full Duplex and Auto Negotiation OFF  */
    
#elif EMAC_LINKSPEED == ETH_LINKSPEED_100
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x00, 0x2100, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** 100M, Full Duplex and Auto Negotiation OFF  */
//    Cy_EthIf_PhyRegWrite(CY_ETH1_TYPE, 0x10, 0x5028, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** Disable auto neg for MDI/MDI-X YOTS in my case this is required **/
    // Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x00, 0x1100, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** 100M, Full Duplex and Auto Negotiation ON  */
    
#elif EMAC_LINKSPEED == ETH_LINKSPEED_1000
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x00, 0x1140, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** 1000M, Full Duplex and Auto Negotiation ON  */
    
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x001F, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** Begin write access to Extended register     */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x0170, PHY_ADDR, CORE_FREQ_IN_MHZ);
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x401F, PHY_ADDR, CORE_FREQ_IN_MHZ);
    u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x0E, PHY_ADDR, CORE_FREQ_IN_MHZ);
    u32ReadData = u32ReadData & 0x0000;                                 /** changing IO impedance on the PHY    */
    u32ReadData = u32ReadData | 0x010C;
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, u32ReadData, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** Enable clock from PHY -> Route it to MCU    */
    u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x0E, PHY_ADDR, CORE_FREQ_IN_MHZ);
#endif
        
    /** Disable RGMII by accessing extended register set || Please read data sheet section 8.4.2.1 for procedure in detail */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x001F, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** REGCR  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x0032, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** ADDAR, 0x0032 RGMII config register  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x401F, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** REGCR, will force next write/read access non incremental  */
#if EMAC_INTERFACE != EMAC_RGMII    
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x0000, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** Disable RGMII  */    
    u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x0E, PHY_ADDR, CORE_FREQ_IN_MHZ);       /** Read the RGMII mode status  */
#else
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x00D3, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** Enable Tx and RX Clock delay in RGMII configuration register  */ 
    
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x001F, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** REGCR  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x0086, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** ADDAR, 0x0086 Delay config register  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x401F, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** REGCR, will force next write/read access non incremental  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x0066, PHY_ADDR, CORE_FREQ_IN_MHZ);                     /** Adjust Tx and Rn Clock delays in PHY  */    
    
#endif
        
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x1F, 0x4000, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** CTRL   */      
    Cy_EthIf_WaitUsec(30000, CORE_FREQ_IN_MHZ);                                                      /** Some more delay to get PHY adapted to new interface	*/      
    u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x11, PHY_ADDR, CORE_FREQ_IN_MHZ);   
    
#if 0
    /** Enable Loop Back Functions; Tested only for RGMII  */
    // Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x10, 0x4000, PHY_ADDR, CORE_FREQ_IN_MHZ);          /** Reset; PHYCR   */    
    
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x001F, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** REGCR  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x00FE, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** ADDAR, 0x00FE Loopback Configuration Register  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x401F, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** REGCR, will force next write/read access non incremental  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0xE720, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** Enable Loopback configurations  */    
    
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x00, 0x4140, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** 1000M, Full Duplex and Auto Negotiation OFF, Loopback enabled   */
    ...
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x16, 0xD001, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** BISCR   */  
    
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x001F, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** REGCR  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x0086, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** ADDAR, 0x0086 Delay config register  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0D, 0x401F, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** REGCR, will force next write/read access non incremental  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x0E, 0x0066, PHY_ADDR, CORE_FREQ_IN_MHZ);             /** Control Clock delay on Tx and Rx Line  */    
    ...
    Cy_EthIf_WaitUsec(5000, CORE_FREQ_IN_MHZ);                                                /** Some more delay to get PHY adapted to new interface	*/    
...
static bool Check_DP83867IR_LinkStatus (void)
{
    ...
    u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x11, PHY_ADDR, CORE_FREQ_IN_MHZ);       /** PHY status register: 0x0011 */
...
static bool Phy_DP83867IR_MDIO_validation (void)
{
    ...
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x19, 0x0777, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** LED ON  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x19, 0x0555, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** LED OFF */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x19, 0x0777, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** LED ON  */  
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x19, 0x0555, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** LED OFF */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x19, 0x4444, PHY_ADDR, CORE_FREQ_IN_MHZ);         /** Restoring default LED configuration, show status of Link and Speed  */
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x18, 0xD032, PHY_ADDR, CORE_FREQ_IN_MHZ);
    
    /** Dummy Read Operation for DP83867IR PHY   */
    u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x02, PHY_ADDR, CORE_FREQ_IN_MHZ);       /** PHY Identifier Register #1  */
    ...
    u32ReadData = Cy_EthIf_PhyRegRead(ETH_REG_BASE, (uint8_t)0x03, PHY_ADDR, CORE_FREQ_IN_MHZ);       /** PHY Identifier Register #2  */
